...
export const verifyUser = asyncHandler(async (req, res) => {
  ...
  const validOTP =
    crypto.createHash("sha512").update(otp).digest("hex") === user.otp;
  if (!validOTP) {
 ...
 * @parameters protocol, hostname, endpoint, and email to genarate forgot password link
 * @return Success message
 ************************************************/

export const forgotPassword = asyncHandler(async (req, res) => {
  const { protocol, hostname, endpoint, email } = req.body;

  if (!protocol || !hostname || !endpoint || !email) {
  ...
  const resetUrl = `${protocol}//${hostname}/${endpoint}/${resetToken}`;
  ...
  try {
    mailSender({
      email,
      subject: `Password reset mail for ${hostname}`,
      text: text,
    });
